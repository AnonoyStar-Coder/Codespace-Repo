...
# Add some space after the Add Product Button
...
# Save Report Button
save_report_button = tk.Button(output_button_frame, text="Save Report", command=lambda: save_report(), bg="#4CAF50", fg="white")
save_report_button.grid(row=0, column=0, padx=5)

# Refresh Button
refresh_button = tk.Button(output_button_frame, text="Refresh", command=refresh_inventory, bg="#2196F3", fg="white")
...
close_button = tk.Button(output_button_frame, text="Close", command=close_application, bg="#F44336", fg="white")
close_button.grid(row=0, column=2, padx=5)

# Create buttons for sorting and operations
button_frame2 = tk.Frame(output_frame, bg="#C8E6C9")
...
def sort_inventory_by_quantity():
    inventory_listbox.delete(0, tk.END)  # Clear current listbox content
    c.execute("SELECT name, quantity FROM products ORDER BY quantity")  # Select and sort products by quantity descending
    rows = c.fetchall()  # Fetch all sorted rows
    for row in rows:
        inventory_listbox.insert(tk.END, f"{row[0]} - {row[1]} units")
