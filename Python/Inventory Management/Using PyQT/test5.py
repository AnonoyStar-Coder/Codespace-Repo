...
from PyQt5.QtGui import QColor  # Ensure this is the only import from PyQt5.QtGui
...
class InventoryApp(QMainWindow):
    def __init__(self):
        ...
        self.add_product_button = self.create_button("Add Product", "green")
        
        # Product Quantity
        self.product_quantity_label = QLabel("Quantity:")
        self.product_quantity_entry = QLineEdit()
        self.update_product_button = self.create_button("Update Product", "#FFB300")  # Yellowish
        ...
        self.search_button = self.create_button("Search", "#0078D7")  # Blue for search
        ...
        self.sort_name_button = self.create_button("Sort by Name", "magenta")
        self.sort_name_button.clicked.connect(lambda: self.sort_inventory("name"))
        
        self.sort_asc_button = self.create_button("Sort Ascending", "magenta")
        self.sort_asc_button.clicked.connect(lambda: self.sort_inventory("asc"))

        self.sort_desc_button = self.create_button("Sort Descending", "magenta")
        ...
        self.delete_product_button = self.create_button("Delete Product", "red")
        self.delete_product_button.clicked.connect(self.delete_product)

        self.save_button = self.create_button("Save to CSV", "blue")  # Blue button for saving
    ...
    def create_button(self, text, color):
        """Create a styled button to resemble Windows 11 buttons."""
        ...
        button.setStyleSheet(f"""
            ...
            QPushButton:pressed {{
                background-color: {self.darker_color(color, 1.5)};
            }}
        """)
        return button

    def darker_color(self, color, factor=1):
