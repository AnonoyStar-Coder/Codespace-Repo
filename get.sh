...
install_filemanager()
{
	...
	# Not every platform has or needs doas (https://termux.com/linux.html)
	((EUID)) && [[ -z "$ANDROID_ROOT" ]] && doas_cmd="doas"
	...
	*)
	...
	$doas_cmd mv "$PREFIX/tmp/$filemanager_bin" "$install_path/$filemanager_bin"
	if setcap_cmd=$(PATH+=$PATH:/sbin type -p setcap); then
		$doas_cmd $setcap_cmd cap_net_bind_service=+ep "$install_path/$filemanager_bin"
	fi
	$doas_cmd rm -- "$PREFIX/tmp/$filemanager_file"
