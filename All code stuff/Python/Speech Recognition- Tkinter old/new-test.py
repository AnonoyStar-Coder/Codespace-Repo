...
status_label.config(text="Click 'Start Listening' to begin")
...
>>> the error is related to google. the error is "cannot acess attribute 'recognize_google' for class Recognizer"
The error message indicates that there's no attribute called `recognize_google` in the `Recognizer` class.

However, it seems that you're trying to use a similar functionality as Google Speech Recognition using `speech_recognition` library. The 
correct method to recognize speech is `recognizer.listen()` followed by `recognizer.recognize()`.
