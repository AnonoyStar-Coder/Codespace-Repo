...
sort_asc_button = ttk.Button(sorting_frame, text="Sort Ascending", command=lambda: sort_inventory(True), style="Rounded.TButton")
sort_asc_button.pack(side=tk.LEFT, padx=5)

sort_desc_button = ttk.Button(sorting_frame, text="Sort Descending", command=lambda: sort_inventory(False), style="Rounded.TButton")
sort_desc_button.pack(side=tk.LEFT, padx=5)

# Function to sort the inventory
def sort_inventory(ascending):
    inventory_listbox.delete(0, tk.END)
    order = "ASC" if ascending else "DESC"
    c.execute(f"SELECT name, quantity FROM products ORDER BY name {order}")
    rows = c.fetchall()
    for row in rows:
        inventory_listbox.insert(tk.END, f"{row[0]} - {row[1]} units")
