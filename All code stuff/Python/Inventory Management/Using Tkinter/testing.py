...
#     # This function updates the stock display in the GUI
#     stock_display.delete(0, tk.END)  # Clear the display list
#     for product, quantity in inventory.items():
#         stock_display.insert(tk.END, f"{product}: {quantity}")
...
# def remove_stock(product, amount):
#     if product in inventory:
...
# # Stock display
# stock_display = tk.Listbox(root)
# stock_display.pack()

# # Buttons for adding/removing stock
# add_button = tk.Button(root, text="Add Stock", command=lambda: add_stock("Product1", 5))
# add_button.pack()

# remove_button = tk.Button(root, text="Remove Stock", command=lambda: remove_stock("Product1", 3))
# remove_button.pack()
...
def add_stock():
    ...
    if result:
        # Update the quantity by adding new stock
        new_quantity = result[0] + int(product_quantity)
        c.execute("UPDATE products SET quantity = ? WHERE name = ?", (new_quantity, product_name))
        conn.commit()
        messagebox.showinfo("Stock Added", f"{product_quantity} units added to {product_name}.")
        show_inventory()
    else:
        messagebox.showwarning("Product Not Found", f"{product_name} does not exist in inventory.")

# New Function: Remove Stock
def remove_stock():
    product_name = product_name_entry.get()
    product_quantity = product_quantity_entry.get()
    ...
    if result:
        if result[0] >= int(product_quantity):
            # Update the quantity by removing stock
            new_quantity = result[0] - int(product_quantity)
            c.execute("UPDATE products SET quantity = ? WHERE name = ?", (new_quantity, product_name))
            conn.commit()
            messagebox.showinfo("Stock Removed", f"{product_quantity} units removed from {product_name}.")
            show_inventory()
        else:
            messagebox.showwarning("Insufficient Stock", f"Not enough stock to remove {product_quantity} units.")
    else:
        messagebox.showwarning("Product Not Found", f"{product_name} does not exist in inventory.")
...
# Buttons for adding/removing stock (ensure they are placed correctly)
add_stock_button = ttk.Button(button_frame, text="Add Stock", command=add_stock, style="Rounded.TButton")
add_stock_button.grid(row=2, column=1, padx=5, pady=5)  # Ensure correct row and column

remove_stock_button = ttk.Button(button_frame, text="Remove Stock", command=remove_stock, style="Rounded.TButton")
remove_stock_button.grid(row=2, column=2, padx=5, pady=5)  # Ensure correct row and column
...
# Add quantity range fields for advanced search (side by side)
quantity_frame = tk.Frame(input_frame, bg="#F9E79F")
quantity_frame.grid(row=7, column=0, pady=5, sticky="w")

min_quantity_label = tk.Label(quantity_frame, text="Min Quantity:", bg="#F9E79F", font=("Helvetica", 12, "bold"))
min_quantity_label.pack(side=tk.LEFT, padx=(0, 5))
min_quantity_entry = tk.Entry(quantity_frame, width=10, font=("Helvetica", 11))
min_quantity_entry.pack(side=tk.LEFT, padx=(0, 20))

max_quantity_label = tk.Label(quantity_frame, text="Max Quantity:", bg="#F9E79F", font=("Helvetica", 12, "bold"))
max_quantity_label.pack(side=tk.LEFT, padx=(0, 5))
max_quantity_entry = tk.Entry(quantity_frame, width=10, font=("Helvetica", 11))
max_quantity_entry.pack(side=tk.LEFT)
...
search_button.grid(row=8, column=0, pady=10)
